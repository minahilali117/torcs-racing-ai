...
class CarState(object):
    ...
    def __init__(self):
        ...
        self.csv_file = "race_data3.csv"
        # Define all fields that will be written in setFromMsg
        self.fields = [
            # Track and Positioning
            "angle",
            "curLapTime",
            "distFromStart",
            "track[0]",
    ...
    def _write_data_async(self, data):
        """Write new data row to CSV file asynchronously"""
        # Create and start a new thread for writing data
        thread = threading.Thread(target=self._write_to_csv, args=(data,))
    ...
    def save_full_dataframe(self):
        """Save the complete DataFrame to CSV file (can be called at game end)"""
        try:
            self.df.to_csv(self.csv_file, index=False)
            print(f"Complete race data saved to {self.csv_file}")
        except Exception as e:
            print(f"Error saving complete race data: {e}")
